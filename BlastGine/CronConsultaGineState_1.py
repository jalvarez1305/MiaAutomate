...
query = """SELECT        id, phone_number, name,GETDATE() as fecha,funel_state,'gine_state_1' as blast
            FROM            CW_Contacts
...
update_query ="""INSERT INTO [dbo].[FunnelHistory]
                ...
                SELECT        id, phone_number, name,GETDATE() as fecha,funel_state,'gine_state_1' as blast
            FROM            CW_Contacts
            WHERE        (custom_attributes_es_prospecto = 1) 
                    AND (custom_attributes_interes_en = 'https://miaclinicasdelamujer.com/gynecologia/')
                    AND funel_state =1
