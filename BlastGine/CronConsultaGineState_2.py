...
template_id = 'HX31362181dff1f373f26c4d470ea1df80'
bot_name = None  # Si no deseas usar un bot, puedes pasar None
query = """
        SELECT        top 20 
			id, phone_number, name,GETDATE() as fecha,funel_state,'gine_state_2' as blast
            FROM            CW_Contacts
            WHERE        (custom_attributes_es_prospecto = 1) 
                    AND (custom_attributes_interes_en = 'https://miaclinicasdelamujer.com/gynecologia/')
                    AND funel_state =2
                    AND datediff(dd,DATEADD(ss, [last_activity_at], '1970-01-01 00:00:00'),getdate())  > 15
...
actualizar_funel_states(query,3)
