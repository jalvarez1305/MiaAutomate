...
def GetFreeTimeForDate(Fecha,Consultorio=6):
    """
    Obtiene el tiempo libre de un consultorio específico desde la base de datos para la fecha solicitada.
    ...
    query = f"""select Top 1 Fecha, 
                            case when DATEDIFF(dd,GETDATE(),Fecha) < 1 then 'Hoy'
                                when DATEDIFF(dd,GETDATE(),Fecha) = 1 then 'Mañana'
                                when DATEDIFF(dd,GETDATE(),Fecha) > 5 then concat('El ',FORMAT(Fecha, 'd \d\e MMMM', 'es-es'))
                                else 'El ' + Name
                            End as Dia
                from [cfg].[Calendario]
                where is_habil=1 and Fecha = '{Fecha}'
            """
    # Ejecutar la consulta y obtener el resultado
    result_siguiente_dia = execute_query(query)
    #consulta para obtener sicuiente cita matutina
    query_matutina = f"""EXEC sp_BuscarPrimerEspacioDisponible '{Fecha}'"""
    ...
    query_vespertino = f"""EXEC sp_BuscarPrimerEspacioVespertino '{Fecha}'"""
    #Obtener la siguiente libre de turno matutino
    ...
    if (matutino is not None and not matutino.empty) or (vespertino is not None and not vespertino.empty):
        ...
        result += f"\nA que horas es mas facil para ti?"
        return result
    else:
        return None
